# src 순차 리팩터링 핸드오프: text/style/variable/component mutation tool definitions 분리

## task

위험도가 낮은 `src/server-tool-definitions.js`를 계속 작게 분리한다. 이번 태스크는 text/style/variable/component mutation 계열 tool definition 11개를 constants 파일로 분리하는 작업이다.

## context

- 이전 태스크에서 core, read/detail, discovery/annotation tool definition이 constants 파일로 분리되었다.
- 이번 태스크는 그 다음 순서인 `update_text`부터 `create_component_set`까지의 schema를 분리했다.
...
- `src/server-tool-definitions.js`는 1,207줄로 여전히 크다.
...
1. `src/server-tool-definitions.js`의 node/import/update 계열 schema를 `src/constants/server-tool-definitions-node.js` 같은 constants 파일로 분리한다.
2. 추천 범위는 `preview_changes`부터 `reuse_or_create_component`까지다.
3. RED 테스트는 `buildToolDefinitions()`에서 mutation 묶음 다음 순서가 유지되는지 확인하는 방식으로 작성한다.
...
`docs/handoff/2026-06-05-src-refactor-tool-definitions-mutation-handoff.md`를 읽고 다음 태스크를 진행해 주세요. 다음 태스크는 `src/server-tool-definitions.js`의 node/import/update 계열 tool definition을 constants 파일로 작게 분리하는 것입니다. 기능 변경 없이 TDD RED/GREEN을 지키고, 완료 후 `npm run check:all`, 리포트, 핸드오프, 분리 파일 문서를 한국어로 갱신해 주세요.
