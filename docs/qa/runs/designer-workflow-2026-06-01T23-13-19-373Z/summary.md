...
- pluginId: page:33276:16484
- cases: 32 total, 32 pass, 0 skip, 0 fail
- transport: healthy
- commandReadiness: ready
- writeReadiness: ready
- pending: 0 commands, 0 results
- recentFailedTotal: 0
- beforeCapture: /Users/im_018/Documents/GitHub/Project/figma_skills/xbridge/docs/qa/runs/designer-workflow-2026-06-01T23-13-19-373Z/captures/before.png
- afterCapture: /Users/im_018/Documents/GitHub/Project/figma_skills/xbridge/docs/qa/runs/designer-workflow-2026-06-01T23-13-19-373Z/captures/after.png

## Release Findings

- Release-required case `DS01` is missing.
...
| DS01 | missing | - |
| L27 | pass | action=locked unsupported safe failure; handling=locked layer 판별/수정 API가 없는 현재 범위에서는 locked 포함 요청을 실제 mutation 전에 preview/skip 리포트로 제한한다. |
| L28 | pass | action=update_node visible=false + readback; handling=자연어 광범위 정리 요청에서는 hidden node id를 skipped로 보고해야 한다. |
| L29 | pass | action=image mask safe limitation; handling=IMAGE/MASK 노드가 확인되지 않으면 crop/fill을 임의 변경하지 않고 전용 fixture가 필요하다고 표시한다. |
| L31 | missing | - |
...
| L15 | Style | pass | action=apply_style invalid; handling=스타일 id/key가 없거나 유효하지 않으면 임의 shadow를 만들지 말고 후보/수동값 안내로 전환한다. |
| L16 | Variable Safety | pass | action=bind_variable invalid; handling=존재하지 않는 variable id에 대해 명확한 실패를 반환해야 한다. |
...
| L27 | Safety | pass | action=locked unsupported safe failure; handling=locked layer 판별/수정 API가 없는 현재 범위에서는 locked 포함 요청을 실제 mutation 전에 preview/skip 리포트로 제한한다. |
| L28 | Hidden Safety | pass | action=update_node visible=false + readback; handling=자연어 광범위 정리 요청에서는 hidden node id를 skipped로 보고해야 한다. |
| L29 | Safety | pass | action=image mask safe limitation; handling=IMAGE/MASK 노드가 확인되지 않으면 crop/fill을 임의 변경하지 않고 전용 fixture가 필요하다고 표시한다. |
