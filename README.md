...
플러그인 안의 AI 디자이너 채팅은 로컬 브리지의 `POST /api/designer/chat`을 통해 동작합니다. 이 API는 먼저 현재 Figma 컨텍스트를 읽고, intent/read plan/suggestion bundle을 만든 뒤, AI API 키가 설정되어 있으면 OpenAI Responses API로 디자인 응답을 생성합니다.

```bash
export XBRIDGE_AI_API_KEY=...
npm start
```

선택 설정값:

- `XBRIDGE_AI_API_KEY` 또는 `OPENAI_API_KEY`: AI 응답 생성에 사용할 API key
- `XBRIDGE_AI_MODEL` 또는 `OPENAI_MODEL`: 사용할 모델명
- `XBRIDGE_AI_BASE_URL` 또는 `OPENAI_BASE_URL`: OpenAI 호환 API base URL
- `XBRIDGE_AI_PROVIDER`: 현재는 `openai`만 지원

API 키가 없으면 채팅이 완전히 멈추지는 않습니다. 대신 `ai.status`가 `unconfigured`로 내려오고, 브리지가 만든 규칙 기반 읽기 결과와 추천 액션만 표시됩니다. 실제 AI 문장 생성까지 테스트하려면 서버 실행 전에 API 키를 설정해야 합니다.

```bash
curl -s -X POST http://127.0.0.1:3846/api/designer/chat \
...
npm run set:keychain-ai -- YOUR_AI_KEY gpt-4.1-mini
npm run start:keychain
```

`set:keychain-token`은 `writable-mcp-bridge/figma-access-token` 항목으로 Figma 토큰을 저장합니다.  
`set:keychain-ai`는 `writable-mcp-bridge/xbridge-ai-api-key`와 관련 모델 설정을 저장합니다.  
`start:keychain`은 이 값들을 읽어 `FIGMA_ACCESS_TOKEN`, `XBRIDGE_AI_API_KEY`, `XBRIDGE_AI_MODEL`, `XBRIDGE_AI_BASE_URL`, `XBRIDGE_AI_PROVIDER` 환경변수로 브리지를 실행합니다.
